...
const EditWorkout: React.FC<EditWorkoutProps> = ({ templateId, onDone, onDeleted }) => {
  ...
  const handleAddExercise = () => {
    ...
    const exercises = [...template.exercises, newExercise];
    updateTemplate({ ...template, exercises });
    setExpandedExerciseIds(prevIds => [...prevIds, newExercise.id]);
  };

  const handleDeleteExercise = (exerciseId: string) => {
    const exercises = template.exercises.filter(ex => ex.id !== exerciseId);
    updateTemplate({ ...template, exercises });
  ...
  const handleToggleEditName = (exercise: Exercise) => {
    if (editingExerciseName?.id === exercise.id) {
        ...
        if(editingExerciseName) { // Save previous one if switching
            const prevExercise = template.exercises.find(ex => ex.id === editingExerciseName.id);
  ...
  const handleReorder = (newOrder: Exercise[]) => {
      updateTemplate({ ...template, exercises: newOrder });
  };

  const handleMoveExercise = (indexToMove: number, direction: 'up' | 'down') => {
      if ((direction === 'up' && indexToMove === 0) || (direction === 'down' && indexToMove === template.exercises.length - 1)) {
          return;
      }
      ...
      updateTemplate({ ...template, exercises: newExercises });
  };
    ...
    <div className="bg-background text-foreground h-screen flex flex-col">
      <div className="flex-grow overflow-y-auto p-4 sm:p-6">
        ...
        <div className="mb-8 max-w-6xl mx-auto">
            <Reorder.Group axis="y" values={template.exercises} onReorder={handleReorder} className="space-y-4">
                <AnimatePresence>
